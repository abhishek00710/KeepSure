import SwiftUI

enum AppTheme {
    static let accent = Color(red: 0.70, green: 0.58, blue: 0.39)
    static let secondaryAccent = Color(red: 0.42, green: 0.36, blue: 0.30)
    static let ink = Color(red: 0.21, green: 0.19, blue: 0.18)
    static let night = Color(red: 0.39, green: 0.34, blue: 0.30)
    static let plum = Color(red: 0.73, green: 0.68, blue: 0.62)
    static let sand = Color(red: 0.97, green: 0.95, blue: 0.92)
    static let mist = Color(red: 0.93, green: 0.91, blue: 0.87)
    static let ivory = Color(red: 0.99, green: 0.98, blue: 0.96)
    static let success = Color(red: 0.42, green: 0.60, blue: 0.46)
    static let warning = Color(red: 0.79, green: 0.58, blue: 0.32)

    static let dashboardGradient = LinearGradient(
        colors: [Color(red: 0.95, green: 0.92, blue: 0.87), Color(red: 0.88, green: 0.83, blue: 0.76)],
        startPoint: .topLeading,
        endPoint: .bottomTrailing
    )

    static let captureGradient = LinearGradient(
        colors: [Color(red: 0.89, green: 0.91, blue: 0.89), Color(red: 0.80, green: 0.84, blue: 0.83)],
        startPoint: .topLeading,
        endPoint: .bottomTrailing
    )

    static let homeBackground = LinearGradient(
        colors: [Color(red: 0.98, green: 0.97, blue: 0.95), Color(red: 0.95, green: 0.93, blue: 0.90), Color(red: 0.93, green: 0.90, blue: 0.86)],
        startPoint: .topLeading,
        endPoint: .bottomTrailing
    )

    static let panelFill = Color.white.opacity(0.88)
    static let elevatedPanelFill = Color.white.opacity(0.52)
}
